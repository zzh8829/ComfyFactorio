...
function Public.reset_wave_defense()
    ...
    this.next_wave = game.tick + 4000 * 15
    this.side_targets = {}
    this.simple_entity_shredding_cost_modifier = 0.009
	local math_random = math.random(-1,1)*2
	
	local positions = {x = 100, y = 0}
	positions.y = math.random(-120,120)
	positions.x = math.random(-120,120)
	if positions.y < 75 and positions.y > -75 then
	
	if positions.y < 0 then
	positions.y = positions.y - 50
	else
	positions.y = positions.y + 50
	end
	end
	...
	positions.x = positions.x - 50
	else
	positions.x = positions.x + 50
	end
	end

    this.spawn_position = positions
    ...
    this.unit_group_command_delay = 4600 * 20
    ...
    this.alert_boss_wave = true
    this.remove_entities = true
    this.enable_side_target = true
    this.enable_threat_log = true
    this.disable_threat_below_zero = true
